...
class LoginVC: UIViewController {
    ...
    func loginLabelInit() {
        ...
        closeButton.frame = CGRect(x: screenSize.width*0.76, y: screenSize.height*0.1, width: 30, height: 30)
        closeButton.setImage(UIImage.init(systemName: "xmark")?.withRenderingMode(.alwaysTemplate), for: .normal)
        closeButton.tintColor = textOneColor
        closeButton.contentVerticalAlignment = .fill
        closeButton.contentHorizontalAlignment = .fill
        closeButton.imageEdgeInsets = UIEdgeInsets(top: 3, left: 2, bottom: 3, right: 2)
    ...
    func registerButtonInit() {
        registerButton.frame = CGRect(x: 0, y: screenSize.height*0.65, width: 80, height: 40)
        registerButton.backgroundColor = backBoxesColor
        registerButton.setTitle("Register", for: .normal)
        registerButton.setTitleColor(UIColor.orange, for: .normal)
        registerButton.titleLabel?.font = UIFont.boldSystemFont(ofSize: 14)
        registerButton.layer.cornerRadius = 20
        registerButton.layer.maskedCorners = [.layerMaxXMinYCorner, .layerMaxXMaxYCorner]
        registerButton.layer.shadowColor = boxesShadowColor.cgColor
        registerButton.layer.shadowOffset = CGSize(width: -1, height: 2)
        registerButton.layer.shadowOpacity = 1
        registerButton.layer.shadowRadius = 0.5
        registerButton.addTarget(self, action: #selector(buttonClicked), for: .touchUpInside)
