    ...
    @objc func buttonClicked(_ sender: AnyObject?) {
      ...
      if sender === cancelButton {
        ...
        if mapView.annotations.count == 0 {
            ...
            let latitude = mapView.annotations[0].coordinate.latitude
            let longitude = mapView.annotations[0].coordinate.longitude
            nc.post(name: Notification.Name("AddLocation"), object: nil, userInfo: ["latitude": latitude, "longitude": longitude])
            dismiss(animated: true, completion: nil)
        }
      }
