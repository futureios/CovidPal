...
class ReviewsVC: UIViewController, ListAdapterDataSource {
    ...
    let collectionView: UICollectionView = {
        ...
        let collectionView = UICollectionView(frame: CGRect(x: 0, y: 44, width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height-44), collectionViewLayout: feedLayout)
            ...
            if snap.hasChild("Reviews") {
                ...
                for review in reviews! {
                    ...
                    self.ref.child("Users").child(userID).observeSingleEvent(of: .value, with: { snapshot in
                        let snap_user = snapshot as! DataSnapshot
                        ...
                        if snap_user.hasChild("ProfilePicUrl") {
                            ...
                            rev.append(ReviewsModel(fullName: fullName, profilePic: profileImage, email: email, review: rv, turnaroundTime: turnaroundTimeString, timeStamp: timeStamp))
                        } else {
                            rev.append(ReviewsModel(fullName: fullName, profilePic: "", email: email, review: rv, turnaroundTime: turnaroundTimeString, timeStamp: timeStamp))
                        }
