...
class MapVC: UIViewController, MKMapViewDelegate {
    ...
    var allAnnotation: [Artwork] = []
    var userLocation: CLLocation = CLLocation.init(latitude: 0, longitude: 0)
    ...
    func addLocationButtonInit() {
        addLocationButton.frame = CGRect(x: screenSize.width-60, y: screenSize.height-(280+UIApplication.topSafeAreaHeight), width: 45, height: 45)
    ...
    func informationButtonInit() {
        informationButton.frame = CGRect(x: screenSize.width-60, y: screenSize.height-(220+UIApplication.topSafeAreaHeight), width: 45, height: 45)
        informationButton.layer.cornerRadius = 10
        informationButton.setImage(#imageLiteral(resourceName: "info").withRenderingMode(.alwaysTemplate), for: .normal)
        informationButton.tintColor = imageColor
        informationButton.contentVerticalAlignment = .fill
        informationButton.contentHorizontalAlignment = .fill
        informationButton.imageEdgeInsets = UIEdgeInsets(top: 14, left: 14, bottom: 14, right: 14)
        informationButton.addBlurEffect()
        informationButton.addTarget(self, action: #selector(buttonClicked), for: .touchUpInside)
    ...
    func detailViewInit(name: String, number: String, description: String, address: String, revNum: Int) {
        ...
        boxReviewsButton.frame = CGRect(x: 18, y: boxDetailLabel.bounds.height+98, width: detailWidth-36, height: 30)
    ...
    func addAnnotation(state:String,uid:String,address:String,description:String,lat:Double,long:Double,name:String,number:String,revNum:Int) {
        ...
        allAnnotation.append(artwork)
        distanceFromLocation(userLocation: userLocation, distance: 3000000 as CLLocationDistance)
    }
    
    func addDistanceAnnotation(state:String,uid:String,address:String,description:String,lat:Double,long:Double,name:String,number:String,revNum:Int) {
        let artwork = Artwork(state: state, uid: uid, address: address, descriptionAddress: description, latitude: lat, longitude: long, name: name, number: number, revNum: revNum, title: "", subtitle: "", coordinate: CLLocationCoordinate2D(latitude: CLLocationDegrees(lat), longitude: CLLocationDegrees(long)))
    ...
    func distanceFromLocation(userLocation: CLLocation, distance: Double) {
        ...
        for annotation in allAnnotation {
            ...
            if distanceFromLocation < distance {
                addDistanceAnnotation(state: annotation.state, uid: annotation.uid, address: annotation.address, description: annotation.descriptionAddress, lat: annotation.latitude, long: annotation.longitude, name: annotation.name, number: annotation.number, revNum: annotation.revNum)
            }
        }
    }
    ...
    @objc func buttonClicked(_ sender: AnyObject?) {
      ...
      if sender === addLocationButton {
        ...
        let alert = UIAlertController(title: "Choose a radius", message: "(Miles)", preferredStyle: .alert)
        ...
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { action in
            if let num = alert.textFields?.first?.text {
                let fval = (num as NSString).floatValue
                let val = fval*1609.34
                self.addRadiusCircle(radius: CLLocationDistance(val))
                self.distanceFromLocation(userLocation: self.userLocation, distance: Double(val))
            }
        }))
    ...
    @objc func onAddLocation(notification: Notification) {
        ...
        addBaseArtwork(location: CLLocationCoordinate2D(latitude: latitude, longitude: longitude))
        addRadiusCircle(location: CLLocationCoordinate2D(latitude: latitude, longitude: longitude), radius: 3000000 as CLLocationDistance)
        mapView.centerToLocation(CLLocation(latitude: latitude, longitude: longitude), latitudinal: 18000000, longitudinal: 18000000)
        self.distanceFromLocation(userLocation: self.userLocation, distance: 3000000 as CLLocationDistance)
    }
    ...
    func reviewsButtonAlert() {
        ...
        alert.addAction(UIAlertAction(title: "Post a Review", style: .default, handler: { action in
            ...
            if user != nil {
                ...
                Toast.show(message: "Please Login", controller: self)
            }
        }))
        self.present(alert, animated: true)
    }
...
extension MapVC: CLLocationManagerDelegate {
     func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        addRadiusCircle(location: currentLocation, radius: 3000000 as CLLocationDistance)
